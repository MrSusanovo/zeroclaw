                    ...
                    if let Ok(v) = serde_json::from_str::<serde_json::Value>(args) {
                        if let Some(cmd) = v.get("command").and_then(|c| c.as_str()) {
                            format!(
                                ": `{}`",
                                if cmd.len() > 200 {
                                    truncate_at_char_boundary(cmd, 200)
                                } else {
                                    cmd
                                }
                            )
                            ...
                            format!(
                                ": {}",
                                if q.len() > 200 {
                                    truncate_at_char_boundary(q, 200)
                                } else {
                                    q
                                }
                            )
...
// Add this new function before process_channel_message:

/// Handles cleanup and logging when an in-flight channel request times out
/// while waiting for the LLM response.
async fn handle_channel_message_timeout(
    ctx: &ChannelRuntimeContext,
    history_key: &str,
    msg: &traits::ChannelMessage,
    ...
    let message_timeout_secs = ctx.message_timeout_secs;
    let max_tool_iterations = ctx.max_tool_iterations;
    let timeout_msg = format!(
        "LLM response timed out after {}s (base={}s, max_tool_iterations={})",
        timeout_budget_secs, message_timeout_secs, max_tool_iterations
    );
...
/// Handles cleanup and logging when an in-flight channel request is cancelled
/// due to a newer message arriving.
async fn handle_cancelled_message(
    msg: &traits::ChannelMessage,
    route: ChannelRouteSelection,
    target_channel: Option<&Arc<dyn Channel>>,
    draft_message_id: Option<&str>,
...
async fn handle_llm_error(
    ...
    if crate::agent::loop_::is_tool_loop_cancelled(&err) || cancellation_token.is_cancelled() {
    ...
    } else if is_context_window_overflow_error(&err) {
        ...
        let should_rollback_user_turn = err
        ...
        let rolled_back = should_rollback_user_turn
            && rollback_orphan_user_turn(ctx, history_key, &msg.content);
        ...
        if let Some(channel) = target_channel.as_ref() {
            if let Some(ref draft_id) = draft_message_id {
                let _ = channel
                    .finalize_draft(&msg.reply_target, draft_id, &format!("⚠️ Error: {err}"))
                    .await;
            } else {
                let _ = channel
                    .send(
                        &SendMessage::new(format!("⚠️ Error: {err}"), &msg.reply_target)
                            .in_thread(msg.thread_ts.clone()),
                    )
                    .await;
            }
...
async fn process_channel_message(
    ...
    if let Some(cli_result) = run_cli_shell_command(&msg.content).await {
        ...
        append_sender_turn(
            ctx.as_ref(),
            &history_key,
            ChatMessage::assistant(&cli_result),
        );
        return;
    }
