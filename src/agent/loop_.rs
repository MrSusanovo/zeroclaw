...
pub(crate) async fn run_tool_call_loop(
    ...
    for iteration in 0..max_iterations {
        ...
        // Check for vision capability early. Extracted to helper for clarity
        // and testability.
        check_vision_compat(provider, provider_name, history)?;
...
pub async fn run_cli_shell_command(cmd: &str) -> Option<String> {
    ...
    let cmd_owned = cmd.trim_start_matches('!').to_string();
    ...
    // Return Some("") if command executed (even with no output), None if not a shell command
    Some(combined)
}

/// Handle interactive CLI slash-commands (/help, /clear, /quit) extracted
...
pub async fn process_slash_command(
    ...
    system_prompt: &str,
    mem: &Arc<dyn Memory>,
) -> Result<Option<SlashCommandOutcome>> {
    match user_input {
        "/quit" | "/exit" => return Ok(Some(SlashCommandOutcome::Quit)),
        "/help" => {
            println!("Available commands:");
            println!("  /help        Show this help message");
            println!("  /clear /new  Clear conversation history");
            println!("  /quit /exit  Exit interactive mode\n");
            return Ok(Some(SlashCommandOutcome::Continue));
        }
        "/clear" | "/new" => {
...
pub async fn run(
    ...
    if let Some(msg) = message {
        ...
        loop {
            ...
            if run_cli_shell_command(&user_input).await.is_some() {
                continue;
            }
            if let Some(outcome) =
                process_slash_command(&user_input, &mut history, &system_prompt, &mem).await?
            {
                match outcome {
                    SlashCommandOutcome::Quit => break,
                    SlashCommandOutcome::Continue => continue,
                }
            }
